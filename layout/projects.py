...
def create_project_layout(lang_data,projects_data):

    layout = [[
        sg.Frame('',[   
            [
                # sg.Button("Select a app to launch",expand_x=True,visible=True,k=f"-selected_app_lbl-",font=FONT,disabled=True)
                sg.Column(requirements_layout.create_layout(lang_data,projects_data), key=PROJECTS_COL_PLACEHOLDER, element_justification='r', expand_x=True,expand_y=True,visible=True),
            ],
